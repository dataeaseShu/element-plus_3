...
ignoreDynamicRequires)
{
  ...
  map(
    (id, index) =>
    `import ${
    id.endsWith('.json') ? `json${index}` : `{ __require as require${index} }`} from ${
    JSON.stringify(id)};`
  ).
  join('\n');
...
function getRequireHandlers() {
  ...
  commonjsMeta)
  {
    ...
    if (dynamicRequireName) {
      imports.push(
        `import { ${
        isDynamicRequireModulesEnabled ? CREATE_COMMONJS_REQUIRE_EXPORT : COMMONJS_REQUIRE_EXPORT} as ${
        dynamicRequireName} } from "${DYNAMIC_MODULES_ID}"`
      );
    }
    if (exportMode === 'module') {
      imports.push(
...
module.exports = Object.assign(exports.default, exports); /* v7-92caa5075b74d0b2 */