...
export const useTree = (
  ...
  const UPDATE_EVENT_CHECK = computed(() => {
    return props.showBtn ? UPDATE_MODEL_EVENT_CACH : UPDATE_MODEL_EVENT
  })
    ...
    onCheck: (data, params) => {
      ...
      // only can select leaf node
        ...
        if (props.multiple) {
          ...
          emit(UPDATE_EVENT_CHECK.value, cachedKeys.concat(childKeys))
        } else {
      ...
      nextTick(() => {
        ...
        !props.showBtn && tree.value.setCheckedKeys(checkedKeys)
